...
CHECK_PARAMETERS_INTERVAL = {
    ...
    'ACTIVATION_DENSE': ['relu', 'tanh', 'elu', 'softmax', 'selu',
                         'softplus', 'softsign', 'sigmoid', 'hard_sigmoid',
                         'exponential', 'linear'],
    'ACTIVATION_CNN': ['relu', 'tanh', 'elu', 'softmax', 'selu',
                       'softplus', 'softsign', 'sigmoid', 'hard_sigmoid',
                       'exponential', 'linear'],
    'ACTIVATE': [True, False],
...
def set_parameters(parameters, architecture, features):
    ...
    check_parameters()
    create_encoding(architecture, features)
    ...
    for key, parameter in PARAMETERS_INTERVAL.items():
        ...
        if parameter['type'] == 'string':
            if len(parameter['value']) == 0:
                raise ValueError(f"GA.SEARCH_SPACE.{key} parameters cannot "
                                 f"be empty."
                                 f"Check the documentation! \n"
                                 f"Options: {CHECK_PARAMETERS_INTERVAL[key]}")

            if not all(item in CHECK_PARAMETERS_INTERVAL[key] for
                       item in parameter['value']):
                raise ValueError(f"GA.SEARCH_SPACE.{key} parameters with "
                                 f"invalid value. "
                                 f"Check the documentation! \n"
                                 f"Options: {CHECK_PARAMETERS_INTERVAL[key]}")

        elif parameter['type'] == 'numbers':
            if min(parameter['value']) < \
                    CHECK_PARAMETERS_INTERVAL[key]['MIN'] or \
                    max(parameter['value']) > \
            ...
            if parameter['value'] < CHECK_PARAMETERS_INTERVAL[key]['MIN'] or \
                    parameter['value'] > CHECK_PARAMETERS_INTERVAL[key]['MAX']:
                raise ValueError(f"GA.SEARCH_SPACE.{key} parameters with "
                                 f"invalid value. "
                                 f"Check the documentation! \n"
...
def create_encoding(architecture, features):
    ...
    if architecture == 'CNN' or architecture == 'CNN/MLP':
        ...
        for i in range(PARAMETERS_INTERVAL['MAX_CNN_LAYERS']['value']):
            for key, value in \
                    PARAMETERS_ARCHITECTURE['LAYER']['CNN_LAYER'].items():
    ...
    for i in range(PARAMETERS_INTERVAL['MAX_DENSE_LAYERS']['value']):
        for key, value in \
                PARAMETERS_ARCHITECTURE['LAYER']['DENSE_LAYER'].items():
            ENCODING[key + '_' + str(i)] = value
