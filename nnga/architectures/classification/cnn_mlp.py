...
class CNN_MLP(BaseNeuralNetwork):
    ...
    def create_model(self):
        ...
        if self.include_top:
            ...
            cnn_mlp = Dense(units=max(C / 2, self.n_classes), activation="relu",)(
                cnn_mlp
            )
            cnn_mlp = Dropout(self._cfg.MODEL.DROPOUT)(cnn_mlp)
            cnn_mlp = Dense(
                units=max(C / 4, self.n_classes), activation="relu",
            )(cnn_mlp)
            cnn_mlp = Dropout(self._cfg.MODEL.DROPOUT)(cnn_mlp)
            cnn_mlp = Dense(
                units=max(C / 8, self.n_classes), activation="relu",
            )(cnn_mlp)
            cnn_mlp = Dropout(self._cfg.MODEL.DROPOUT)(cnn_mlp)
            cnn_mlp = Dense(units=self.n_classes, activation="softmax",)(
                cnn_mlp
            )
